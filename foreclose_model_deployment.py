...
st.markdown(
    ...
    /* Primary button (Predict) - emphasized like title */
...
def load_artifacts(artifacts_dir: Path):
    ...
    lot_area_q75 = joblib.load(artifacts_dir / "lot_area_q75.pkl")
    scaler = joblib.load(artifacts_dir / "scaler.pkl")

    models = {
        "Linear Reg": joblib.load(artifacts_dir / "linear_reg.pkl"),
        "Ridge (Tuned)": joblib.load(artifacts_dir / "ridge_tuned.pkl"),
        "Lasso (Tuned)": joblib.load(artifacts_dir / "lasso_tuned.pkl"),
        "Random Forest (Tuned)": joblib.load(artifacts_dir / "rf_tuned.pkl"),
        "Gradient Boosting (Tuned)": joblib.load(artifacts_dir / "gb_tuned.pkl"),
    }
...
# ==========================
if not ART.exists():
    st.error(f"Artifacts folder not found: {ART.resolve()}")
    st.stop()

feature_cols, lot_area_q75, scaler, models = load_artifacts(ART)
...
with col1:
    ...
    predict_clicked = st.button(
        ...
        type="primary"  # supported button type [web:151]
    )
...
with col2:
    ...
    if pred_table is None:
        ...
        with m1:
            st.metric("Max (across models)", f"₱{p_max:,.2f}")
        with m2:
            st.metric("Median (across models)", f"₱{p_med:,.2f}")
        with m3:
            st.metric("Min (across models)", f"₱{p_min:,.2f}")
