...
def pie_variance_chart(pred_table: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(
        go.Pie(
            labels=pred_table["Model"],
            values=pred_table["Pred_price"],
            hole=0.55,
    ...
    fig.update_layout(
        title=dict(text="Prediction Share by Model",
                   font=dict(size=14, family="Poppins", color="#0F172A")),
        showlegend=False,
        plot_bgcolor="white", paper_bgcolor="white",
...
with col1:
    ...
    st.markdown(
        ...
        "Lot Area ≥ Floor Area. Max lot: 50,000 sqm / Max floor: 20,000 sqm.</div>",
        unsafe_allow_html=True,
    )
...
with col2:
    ...
    if pred_table is None:
        ...
        with vc2:
            st.plotly_chart(pie_variance_chart(pred_table), use_container_width=True)
