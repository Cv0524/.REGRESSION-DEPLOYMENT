...
st.markdown(
    ...
    .spread-box td { padding: 6px 10px; border-bottom: 1px solid #E2E8F0; }
    .spread-box tr:last-child td { border-bottom: none; }
...
MAX_LOT_AREA    = 1_000.0
MAX_FLOOR_AREA  = 1_000.0
WARN_LOT_AREA   = 800.0
...
# Spread Info Helper
# ==========================
def render_spread_info(p_min: float, p_med: float, p_max: float, spread_pct: float):
    """Renders an expander explaining what spread means with a live example."""
    with st.expander("📐 What does Spread % mean?", expanded=False):
        st.markdown(
            f"""
            <div class="spread-box">
                <strong>Spread %</strong> measures how much the 5 prediction models
                <em>disagree</em> with each other. A low spread means all models
                are confident and aligned; a high spread means you should treat
                the result as a rough estimate only.<br><br>
                <strong>Formula:</strong><br>
                <code>Spread % = (Max Price − Min Price) / Median Price × 100</code>
                <br><br>
                <strong>Your current result:</strong><br>
                <code>(₱{p_max:,.2f} − ₱{p_min:,.2f}) / ₱{p_med:,.2f} × 100
                = <b>{spread_pct:.1f}%</b></code>
                <br><br>
                <table>
                    <tr>
                        <th>Spread %</th>
                    ...
                    <tr>
                        ...
                        <td>All models agree — use any predicted value confidently</td>
                    </tr>
                    <tr>
                        <td>10% – 30%</td>
                        <td>🟡 Moderate Spread</td>
                        <td>Some disagreement — rely on the <strong>Median</strong> price</td>
                    </tr>
                    <tr>
                        <td>Above 30%</td>
                        <td>🔴 High Spread</td>
                        <td>Models strongly disagree — treat as a wide price range</td>
                    </tr>
                </table>
            </div>
            """,
...
with col2:
    ...
    if pred_table is None:
        ...
        # ← NEW: Spread info expander with live values
        render_spread_info(p_min, p_med, p_max, spread_pct)
